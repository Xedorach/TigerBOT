...
int LineDetect::colorthresh(cv::Mat input) {
  ...
  LineDetect::LowerWhite = {0, 0, 173};
  LineDetect::UpperWhite = {179, 92, 255};
  cv::inRange(LineDetect::img_hsv, LowerWhite,
   UpperWhite, LineDetect::img_mask);
  img_mask(cv::Rect(0, 0, w, 0.8*h)) = 0;
  // Find contours for better visualization
  cv::findContours(LineDetect::img_mask, v,
